...
struct combination {
    ...
    combination(int n_) : n(n_) {
        ...
        inv.resize(n+1);
        pmt[0] = 1;
        rep(i, 1, n+1) pmt[i] = pmt[i-1] * i;
        inv[n] = pmt[n].inv();
        repr(i, 0, n) inv[i] = inv[i+1] * (i+1);
    }
    mint get(int a, int b) {
        if (a > n || a < b || b < 0) return 0;
        return pmt[a] * inv[b] * inv[a-b];
    }
};
